...
class FluxImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16):
        ...
        self.model_names = ['text_encoder_1', 'text_encoder_2', 'dit', 'vae_decoder', 'vae_encoder', 'controlnet',
                            'ipadapter', 'ipadapter_image_encoder', 'qwenvl', 'step1x_connector']
    ...
    def fetch_models(self, model_manager: ModelManager, controlnet_config_units: List[ControlNetConfigUnit] = [],
                     prompt_refiner_classes=[], prompt_extender_classes=[]):
    ...
    def from_model_manager(model_manager: ModelManager, controlnet_config_units: List[ControlNetConfigUnit] = [],
                           prompt_refiner_classes=[], prompt_extender_classes=[], device=None, torch_dtype=None):
    ...
    def preprocess_masks(self, masks, height, width, dim, train=False):
        out_masks = []
        for mask in masks:
            if isinstance(mask, torch.Tensor):
                ...
                mask = self.preprocess_image(mask.resize((width, height), resample=Image.NEAREST)).mean(dim=1,
                                                                                                        keepdim=True) > 0
    ...
    def prepare_entity_inputs(self, entity_prompts, entity_masks, width, height, t5_sequence_length=512,
                              enable_eligen_inpaint=False, train=False):
        fg_mask, bg_mask = None, None
        if enable_eligen_inpaint:
            masks_ = deepcopy(entity_masks)
            fg_masks = torch.cat(
                [self.preprocess_image(mask.resize((width // 8, height // 8))).mean(dim=1, keepdim=True) for mask in
                 masks_])
        ...
        entity_prompts = self.encode_prompt(entity_prompts, t5_sequence_length=t5_sequence_length)[
    ...
    def prepare_ipadapter(self, ipadapter_images, ipadapter_scale):
        if ipadapter_images is not None:
            ...
            ipadapter_kwargs_list_posi = {
                "ipadapter_kwargs_list": self.ipadapter(ipadapter_image_encoding, scale=ipadapter_scale)}
            ipadapter_kwargs_list_nega = {
                "ipadapter_kwargs_list": self.ipadapter(torch.zeros_like(ipadapter_image_encoding))}
        else:
            ipadapter_kwargs_list_posi, ipadapter_kwargs_list_nega = {"ipadapter_kwargs_list": {}}, {
                "ipadapter_kwargs_list": {}}
        return ipadapter_kwargs_list_posi, ipadapter_kwargs_list_nega

    def prepare_controlnet(self, controlnet_image, masks, controlnet_inpaint_mask, tiler_kwargs,
                           enable_controlnet_on_negative):
        if controlnet_image is not None:
            self.load_models_to_device(['vae_encoder'])
            controlnet_kwargs_posi = {
                "controlnet_frames": self.prepare_controlnet_input(controlnet_image, controlnet_inpaint_mask,
                                                                   tiler_kwargs)}
            if len(masks) > 0 and controlnet_inpaint_mask is not None:
                print("The controlnet_inpaint_mask will be overridden by masks.")
                local_controlnet_kwargs = [
    ...
    def prepare_eligen(self, prompt_emb_nega, eligen_entity_prompts, eligen_entity_masks, width, height,
                       t5_sequence_length, enable_eligen_inpaint, enable_eligen_on_negative, cfg_scale, train=False):
        if eligen_entity_masks is not None:
            entity_prompt_emb_posi, entity_masks_posi, fg_mask, bg_mask = self.prepare_entity_inputs(
                eligen_entity_prompts, eligen_entity_masks, width, height, t5_sequence_length, enable_eligen_inpaint,
                train=train)
            if enable_eligen_on_negative and cfg_scale != 1.0:
                entity_prompt_emb_nega = prompt_emb_nega['prompt_emb'].unsqueeze(1).repeat(1,
                                                                                           entity_masks_posi.shape[1],
                                                                                           1, 1)
    ...
    def prepare_prompts(self, prompt, local_prompts, masks, mask_scales, t5_sequence_length, negative_prompt,
                        cfg_scale):
        # Extend prompt
        ...
        prompt_emb_nega = self.encode_prompt(negative_prompt, positive=False,
                                             t5_sequence_length=t5_sequence_length) if cfg_scale != 1.0 else None
        prompt_emb_locals = [self.encode_prompt(prompt_local, t5_sequence_length=t5_sequence_length) for prompt_local in
                             local_prompts]
    ...
    def prepare_infinite_you(self, id_image, controlnet_image, infinityou_guidance, height, width):
        if self.infinityou_processor is not None and id_image is not None:
            return self.infinityou_processor.prepare_infinite_you(self.image_proj_model, id_image, controlnet_image,
                                                                  infinityou_guidance, height, width)
        else:
            return {}, controlnet_image

    def prepare_flex_kwargs(self, latents, flex_inpaint_image=None, flex_inpaint_mask=None, flex_control_image=None,
                            flex_control_strength=0.5, flex_control_stop=0.5, tiled=False, tile_size=64,
                            tile_stride=32):
        if self.dit.input_dim == 196:
            if flex_inpaint_image is None:
                flex_inpaint_image = torch.zeros_like(latents)
            else:
                flex_inpaint_image = self.preprocess_image(flex_inpaint_image).to(device=self.device,
                                                                                  dtype=self.torch_dtype)
                flex_inpaint_image = self.encode_image(flex_inpaint_image, tiled=tiled, tile_size=tile_size,
            ...
            if flex_inpaint_mask is None:
                ...
                flex_inpaint_mask = self.preprocess_image(flex_inpaint_mask).to(device=self.device,
                                                                                dtype=self.torch_dtype)
            ...
            if flex_control_image is None:
                ...
                flex_control_image = self.preprocess_image(flex_control_image).to(device=self.device,
                                                                                  dtype=self.torch_dtype)
                flex_control_image = self.encode_image(flex_control_image, tiled=tiled, tile_size=tile_size,
                                                       tile_stride=tile_stride) * flex_control_strength
            ...
            flex_control_stop_timestep = self.scheduler.timesteps[
                int(flex_control_stop * (len(self.scheduler.timesteps) - 1))]
            flex_kwargs = {"flex_condition": flex_condition, "flex_uncondition": flex_uncondition,
    ...
    def prepare_step1x_kwargs(self, prompt, negative_prompt, image):
        ...
        return {"step1x_llm_embedding": embs[0:1], "step1x_mask": masks[0:1], "step1x_reference_latents": image}, {
    ...
    def __call__(
        ...
        # Prompt
        prompt_emb_posi, prompt_emb_nega, prompt_emb_locals = self.prepare_prompts(prompt, local_prompts, masks,
                                                                                   mask_scales, t5_sequence_length,
        ...
        # InfiniteYou
        infiniteyou_kwargs, controlnet_image = self.prepare_infinite_you(infinityou_id_image, controlnet_image,
                                                                         infinityou_guidance, height, width)

        # Entity control
        eligen_kwargs_posi, eligen_kwargs_nega, fg_mask, bg_mask = self.prepare_eligen(prompt_emb_nega,
                                                                                       eligen_entity_prompts,
                                                                                       eligen_entity_masks, width,
                                                                                       height, t5_sequence_length,
                                                                                       enable_eligen_inpaint,
                                                                                       enable_eligen_on_negative,
                                                                                       cfg_scale)

        # IP-Adapter
        ipadapter_kwargs_list_posi, ipadapter_kwargs_list_nega = self.prepare_ipadapter(ipadapter_images,
                                                                                        ipadapter_scale)

        # ControlNets
        controlnet_kwargs_posi, controlnet_kwargs_nega, local_controlnet_kwargs = self.prepare_controlnet(
            controlnet_image, masks, controlnet_inpaint_mask, tiler_kwargs, enable_controlnet_on_negative)
        ...
        flex_kwargs = self.prepare_flex_kwargs(latents, flex_inpaint_image, flex_inpaint_mask, flex_control_image,
                                               ...
                                               flex_control_stop=flex_control_stop, **tiler_kwargs)

        # Step1x
        step1x_kwargs_posi, step1x_kwargs_nega = self.prepare_step1x_kwargs(prompt, negative_prompt,
                                                                            image=step1x_reference_image)

        # TeaCache
        tea_cache_kwargs = {"tea_cache": TeaCache(num_inference_steps,
                                                  rel_l1_thresh=tea_cache_l1_thresh) if tea_cache_l1_thresh is not None else None}
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            inference_callback = lambda prompt_emb_posi, controlnet_kwargs: lets_dance_flux(
                ...
                hidden_states=latents, timestep=timestep, return_attn_maps=(progress_id==len(self.scheduler.timesteps) - 1) and return_attn_maps,
                **prompt_emb_posi, **tiler_kwargs, **extra_input, **controlnet_kwargs, **ipadapter_kwargs_list_posi,
                **eligen_kwargs_posi, **tea_cache_kwargs, **infiniteyou_kwargs, **flex_kwargs, **step1x_kwargs_posi,
            )
            if progress_id==len(self.scheduler.timesteps) - 1 and return_attn_maps:
                inference_callback, joint_attn, single_attn = inference_callback
            ...
            # Inpaint
            if enable_eligen_inpaint:
                noise_pred_posi = self.inpaint_fusion(latents, input_latents, noise_pred_posi, fg_mask, bg_mask,
                                                      progress_id)

            # Classifier-free guidance
            if cfg_scale != 1.0:
                # Negative side
                if self.dit.attn_store:
                ...
                if self.dit.attn_store:
                    self.dit.attn_store.enable_store(True)
                noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega)
            else:
                noise_pred = noise_pred_posi

            if progress_id < len(self.scheduler.timesteps) - 1 and self.dit.attn_store:
                self.dit.attn_store.aggregate_attention()
            # Iterate
        ...
        if return_attn_maps:
            return image, joint_attn, single_attn
        return image
...
class InfinitYou:
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.app_640 = FaceAnalysis(name='antelopev2', root=insightface_root_path,
                                    providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.app_640.prepare(ctx_id=0, det_size=(640, 640))
        self.app_320 = FaceAnalysis(name='antelopev2', root=insightface_root_path,
                                    providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        self.app_320.prepare(ctx_id=0, det_size=(320, 320))
        self.app_160 = FaceAnalysis(name='antelopev2', root=insightface_root_path,
                                    providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
...
def lets_dance_flux(
    ...
    if tiled:
        def flux_forward_fn(hl, hr, wl, wr):
            tiled_controlnet_frames = [f[:, :, hl: hr, wl: wr] for f in
                                       controlnet_frames] if controlnet_frames is not None else None
    ...
    if controlnet is not None and controlnet_frames is not None:
        ...
        if id_emb is not None:
            controlnet_text_ids = torch.zeros(id_emb.shape[0], id_emb.shape[1], 3).to(device=hidden_states.device,
    ...
    if entity_prompt_emb is not None and entity_masks is not None:
        prompt_emb, image_rotary_emb, attention_mask = dit.process_entity_masks(hidden_states, prompt_emb,
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.single_blocks):
            ...
            # ControlNet
            if controlnet is not None and controlnet_frames is not None:
                hidden_states[:, prompt_emb.shape[1]:] = hidden_states[:, prompt_emb.shape[1]:] + \
